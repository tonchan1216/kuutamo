	...
	function customize_register( $wp_customize ){
		...
		if( method_exists($wp_customize, 'add_panel') ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'puro' ),
				'description' => __( 'Change layouts for various pages on your site.', 'puro' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'puro' ),
			'search' => __( 'Search Results', 'puro' ),
			'date' => __( 'Date Archives', 'puro' ),
			'author' => __( 'Author Archives', 'puro' ),
			'404' => __( 'Not Found', 'puro' ),
		);
		...
		foreach( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'puro' ) . ': ' . $post_type_data->label
			);
		}
		...
		foreach( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'puro' ) . ': ' . $taxonomy->label
			);
		}
